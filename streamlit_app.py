...
if arquivo:
    try:
        ...
        # Definir colunas manualmente com base na estrutura do seu arquivo
        df.columns = ["Localizador", "Categoria", "Data_Hora"]

        # Remover linhas vazias ou mal formatadas
        df = df.dropna(subset=["Categoria", "Data_Hora"], how="all")

        # Filtrar apenas as linhas com dados válidos (3 colunas preenchidas)
        df = df[df[["Localizador", "Categoria", "Data_Hora"]].notnull().all(axis=1)]

        # Converter data/hora - tenta automaticamente detectar o formato correto
        ...
        periodo = f"(de {data_inicio.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')})" if len(datas_disponiveis) > 0 else ""
        st.subheader(f"Resumo de Acessos {periodo}")
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if len(datas_disponiveis) > 1:
                ...
                resumo_diario.index = resumo_diario.index.map(lambda x: x.strftime('%d/%m/%Y'))
